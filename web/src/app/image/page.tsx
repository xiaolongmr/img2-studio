...
const imageAutoResolutionPresets: ImageResolutionPreset[] = [
  { tier: "auto", label: "Auto", value: "auto", access: "free" },
];

const HISTORY_SIDEBAR_MIN_WIDTH = 108;
const HISTORY_SIDEBAR_MAX_WIDTH = 560;
const HISTORY_SIDEBAR_DEFAULT_WIDTH = 320;
const HISTORY_SIDEBAR_COMPACT_BREAKPOINT = 160;
...
const imageQualityOptions: Array<{
  ...
  },
  {
    label: "Auto",
    value: "auto",
    description: "交给上游自动判断质量，速度和费用由上游决定",
  },
];

const imageOutputFormatOptions: Array<{
  label: string;
...
function clampHistorySidebarWidth(width: number) {
  return Math.min(
    HISTORY_SIDEBAR_MAX_WIDTH,
    Math.max(HISTORY_SIDEBAR_MIN_WIDTH, Math.round(width)),
  );
}

function clampDesktopPromptHeight(height: number) {
  return Math.min(
    DESKTOP_PROMPT_MAX_HEIGHT,
    Math.max(DESKTOP_PROMPT_MIN_HEIGHT, Math.round(height)),
  );
}
...
type RequestTimerStatus = "idle" | "running" | "success" | "error";
...
export default function ImagePage() {
  ...
  const directActiveRequestsRef = useRef<
    Record<string, RunningActiveRequestState>
  >({});
  ...
  const historyResizeListenersRef = useRef<{
  ...
  const [imageOutputFormat, setImageOutputFormat] =
    useState<ImageOutputFormat>("jpeg");
  ...
  const promptResizeListenersRef = useRef<{
    move: (event: MouseEvent) => void;
    up: () => void;
  } | null>(null);
  ...
  const desktopHistorySidebarWidth = clampHistorySidebarWidth(
    historySidebarWidth,
  );
  const isHistoryCompact =
    desktopHistorySidebarWidth <= HISTORY_SIDEBAR_COMPACT_BREAKPOINT;
  const desktopGridStyle = useMemo<CSSProperties | undefined>(() => {
    if (isStandaloneHistory || isStandaloneWorkspace || historyCollapsed) {
      return undefined;
    }
    return {
      gridTemplateColumns: `${desktopHistorySidebarWidth}px minmax(0, 1fr)`,
    };
  }, [
    desktopHistorySidebarWidth,
    historyCollapsed,
    isStandaloneHistory,
      ...
      currentResolutionPresets.map((item) => ({
        label:
          imageAspectRatio === "auto"
            ? item.label
            : formatResolutionLabel(item.value),
        value: item.tier,
        disabled: false,
      })),
      ...
      mode === "edit" ? (
        <>
          <div>
            ...
            编辑模式会按当前尺寸参数请求 `image2.0`，最终比例仍可能受源图和遮罩范围影响。
          </div>
          <div className="mt-2">
            <span className="font-semibold text-stone-800">质量说明：</span>
            Low 更快，Medium 均衡，High 更慢且更贵，Auto 交给上游判断。
          </div>
        </>
      ) : (
        <>
          <div>
            <span className="font-semibold text-stone-800">尺寸：</span>
            默认方图速度最快；4K 可选 `3840x2160` 横图或 `2160x3840` 竖图，属于高耗时/高成本实验尺寸。
          </div>
          <div className="mt-2">
          ...
          <div className="mt-2">
            ...
            JPEG 体积小适合默认下载，PNG 是无损输出适合二次编辑，WebP 体积更小。
          </div>
        </>
      ),
    [mode],
  ...
  const handleHistoryResizeMouseDown = useCallback(
    (event: ReactMouseEvent<HTMLButtonElement>) => {
      if (!isDesktopLayout || historyCollapsed) {
        return;
      }
      const existingListeners = historyResizeListenersRef.current;
      if (existingListeners) {
        window.removeEventListener("mousemove", existingListeners.move);
        window.removeEventListener("mouseup", existingListeners.up);
        historyResizeListenersRef.current = null;
      }
      historyResizeStateRef.current = {
        startX: event.clientX,
        startWidth: desktopHistorySidebarWidth,
      };
      ...
      const handleMouseMove = (moveEvent: MouseEvent) => {
        ...
        setHistorySidebarWidth(
          clampHistorySidebarWidth(resizeState.startWidth + deltaX),
        );
      };
      const handleMouseUp = () => {
      ...
      historyResizeListenersRef.current = {
    ...
    [desktopHistorySidebarWidth, historyCollapsed, isDesktopLayout],
  );

  const handleDesktopPromptResizeMouseDown = useCallback(
    (event: ReactMouseEvent<HTMLButtonElement>) => {
      if (!isDesktopLayout) {
        return;
      }
      const existingListeners = promptResizeListenersRef.current;
      if (existingListeners) {
        window.removeEventListener("mousemove", existingListeners.move);
        window.removeEventListener("mouseup", existingListeners.up);
        promptResizeListenersRef.current = null;
      }
      promptResizeStateRef.current = {
        startY: event.clientY,
        startHeight: desktopPromptHeight,
      };
      ...
      const handleMouseMove = (moveEvent: MouseEvent) => {
        ...
        setDesktopPromptHeight(
          clampDesktopPromptHeight(resizeState.startHeight - deltaY),
        );
      };
      const handleMouseUp = () => {
      ...
      window.addEventListener("mousemove", handleMouseMove);
      window.addEventListener("mouseup", handleMouseUp);
      promptResizeListenersRef.current = {
        move: handleMouseMove,
        up: handleMouseUp,
      };
    ...
    [desktopPromptHeight, isDesktopLayout],
  );

  useEffect(() => {
    return () => {
      const listeners = historyResizeListenersRef.current;
      if (listeners) {
  ...
  useEffect(() => {
    return () => {
      const listeners = promptResizeListenersRef.current;
      if (listeners) {
        window.removeEventListener("mousemove", listeners.move);
        window.removeEventListener("mouseup", listeners.up);
      }
  ...
  useEffect(() => {
    ...
    document.body.style.cursor = "";
    document.body.style.userSelect = "";
  }, [historyCollapsed, isDesktopLayout]);
  ...
  useEffect(() => {
    ...
    const updateElapsed = () => {
      ...
      setSubmitElapsedSeconds(elapsed);
  ...
  const handleRequestStart = useCallback((request: ActiveRequestState) => {
    const startedAt = Date.now();
    directActiveRequestsRef.current = {
      ...directActiveRequestsRef.current,
      [request.turnId]: {
        ...request,
        startedAt,
    ...
    syncDirectActiveRequests();
    const longestElapsedSeconds = Math.max(
      0,
      Math.floor(
        Math.max(
          ...Object.values(directActiveRequestsRef.current).map(
            (activeRequest) => startedAt - activeRequest.startedAt,
          ),
        ) / 1000,
      ),
    );
    setSubmitElapsedSeconds(longestElapsedSeconds);
    setRequestTimer({ status: "running", elapsedSeconds: longestElapsedSeconds });
  }, [syncDirectActiveRequests]);

  const handleRequestFinish = useCallback((turnId: string, status: "success" | "error") => {
    const activeRequest = directActiveRequestsRef.current[turnId];
    const startedAt = activeRequest?.startedAt ?? null;
    const elapsedSeconds = startedAt
      ? Math.max(0, Math.floor((Date.now() - startedAt) / 1000))
      : 0;
    const remainingRequests = { ...directActiveRequestsRef.current };
    delete remainingRequests[turnId];
    directActiveRequestsRef.current = remainingRequests;
    syncDirectActiveRequests();
    setSubmitElapsedSeconds(elapsedSeconds);
    if (Object.keys(remainingRequests).length > 0) {
      const now = Date.now();
      const longestElapsedSeconds = Math.max(
        0,
        Math.floor(
          Math.max(
            ...Object.values(remainingRequests).map(
              (request) => now - request.startedAt,
            ),
          ) / 1000,
        ),
      );
    ...
    setRequestTimer({ status, elapsedSeconds });
  }, [syncDirectActiveRequests]);

  const timerLabel = useMemo(() => {
    const elapsedSeconds =
      requestTimer.status === "running"
        ? submitElapsedSeconds
    ...
    if (requestTimer.status === "success") {
      return `完成 ${formatTimerClock(elapsedSeconds)}`;
    }
    if (requestTimer.status === "error") {
      return `失败 ${formatTimerClock(elapsedSeconds)}`;
    }
    return "就绪";
  }, [
    effectiveActiveRequests.length,
  ...
  const handleCancelTurn = useCallback(
    async (conversationId: string, turn: ImageConversationTurn) => {
      void conversationId;
      void turn;
      toast.info("当前为直连 NewAPI 请求，提交后暂不支持前端取消");
    },
    [],
  );
  ...
  const historyPanel = (
    <div className="relative lg:min-h-0">
      <HistorySidebar
        ...
        compact={isHistoryCompact && isDesktopLayout && !isStandaloneHistory}
      />
      {!isStandaloneHistory && isDesktopLayout && !historyCollapsed ? (
        <button
          type="button"
          aria-label="拖拽调整历史记录栏宽度"
          title="拖拽调整历史记录栏宽度"
          onMouseDown={handleHistoryResizeMouseDown}
          className="group absolute right-0 top-1/2 z-20 hidden h-32 w-4 -translate-y-1/2 translate-x-full cursor-col-resize items-center justify-start lg:flex"
        >
          <span className="h-28 w-1.5 rounded-l-none rounded-r-full bg-stone-300/95 shadow-[inset_0_1px_1px_rgba(255,255,255,0.5)] transition group-hover:bg-stone-500/95 dark:bg-[var(--studio-border)] dark:group-hover:bg-[var(--studio-text-muted)]" />
        </button>
      ) : null}
    </div>
  );
  ...
  const workspacePanel = (
    ...
    >
      <WorkspaceHeader
        ...
        timerStatus={requestTimer.status}
      ...
      <div className="relative hidden px-5 pb-0 pt-0 lg:flex lg:items-start">
        <button
          type="button"
          onMouseDown={handleDesktopPromptResizeMouseDown}
          className="group absolute left-1/2 top-0 z-10 inline-flex h-5 w-64 -translate-x-1/2 -translate-y-1/2 cursor-row-resize items-center justify-center rounded-full"
          aria-label="拖拽调整输入区高度"
          title="拖拽调整输入区高度"
        >
          <span className="h-2 w-48 rounded-t-full rounded-b-none bg-stone-300/95 shadow-[inset_0_1px_1px_rgba(255,255,255,0.5)] transition group-hover:bg-stone-500/95 dark:bg-[var(--studio-border)] dark:group-hover:bg-[var(--studio-text-muted)]" />
        </button>
      </div>
      ...
      <PromptComposer
        ...
        onUpdateSourceImageReference={updateSourceImageReference}
        ...
        desktopPromptHeight={desktopPromptHeight}
